 ...
 *         4.等游戏的基本功能
 * 需要增强的地方----  游戏的数据安全
 *      介于JS是明文，通过阅读代码可以很容易的的修改游戏中的数据
 *      下一个版本的修改方向：闭包，将游戏中的变量参数放置在闭包中，
 *         只提供事件监听函数修改这些参数的方法，屏蔽外界对内部数据的修改方式
 */
...
var game = {
    ...
    // 检测游戏是否 Over
    ifGmaeOver: function () {
        if (this.ifHasBlock() || this.ifCanMoveRight() || this.ifCanMoveLeft() || this.ifCanMoveDown() || this.ifCanMoveUp()) {
            return false
        }
        return true;
    },
